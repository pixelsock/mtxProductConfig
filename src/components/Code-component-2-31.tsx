...
export function QuoteModal({ quoteItems, onClose, onSubmit }: QuoteModalProps) {
                ...
                <Card key={item.id} className="p-4">
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    <p>Size: {item.configuration.useCustomSize 
                      ? `${item.configuration.customWidth}" × ${item.configuration.customHeight}"`
                      : item.configuration.size.replace('x', '" × "') + '"'
                    }</p>
                    <p>Frame: {item.configuration.frameStyle.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
                    <p>Light: {item.configuration.lightColor.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
                    <p>Mount: {item.configuration.mounting.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
                  </div>
                </Card>
              ))}
            </div>
