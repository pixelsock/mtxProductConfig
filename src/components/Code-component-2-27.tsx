        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div>
                <h4 className="font-semibold text-gray-900">Future Dots Mirror</h4>
                <div className="flex items-center space-x-2">
                  <span className="text-sm text-gray-600">
                    {configuration.frameStyle.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())} • 
                    {configuration.useCustomSize 
                      ? ` ${configuration.customWidth}" × ${configuration.customHeight}"`
                      : ` ${configuration.size.replace('x', '" × "')}"
                    `}
                  </span>
                  <Button
                    variant="ghost"
