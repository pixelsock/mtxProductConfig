  ...
  return {
      ...
      if (product && !isAdjustingSelections && !isProcessingProductUpdate) {
        ...
        if (currentProductLine && currentConfig) {
          ...
          try {
            ...
            const { fetchProductOptions } = await import(
    ...
    resetConfiguration: async () => {
      ...
      const initialConfig: ProductConfig = {
        ...
        hangingTechnique:
          productOptions.hangingTechniques[0]?.id.toString() || "",
      ...
      try {
        ...
        if (finalConfig && productOptions) {
          try {
            const { findBestMatchingProduct } = await import(
    ...
    searchBySku: async (query: string) => {
      ...
      set((state) => ({
        ...state,
        skuSearchQuery: query,
        isSearchingSku: true,
        skuSearchError: null,
      }));

      try {
        const { parseSkuToConfiguration } = await import(
          "../../services/sku-parser"
        );
        const response = await parseSkuToConfiguration(trimmed);

        if (import.meta.env.DEV) {
          response.warnings.forEach((warning) =>
            console.warn("SKU search warning:", warning),
          );
          response.errors.forEach((errorMessage) =>
            console.warn("SKU search error:", errorMessage),
          );
        }
        ...
        set((state) => ({
          ...state,
          skuSearchQuery: query,
          skuSearchResults: response.results,
          isSearchingSku: false,
          skuSearchError:
        ...
        const message =
          error instanceof Error ? error.message : "Failed to search by SKU.";

        set((state) => ({
          ...state,
          isSearchingSku: false,
          skuSearchError: message,
          skuSearchResults: [],
        }));
    ...
    applySkuResult: async (result: SkuSearchResult) => {
      if (
        ...
        set((state) => ({
          ...state,
          skuSearchError:
            "Select a SKU result with resolved segments before applying.",
        }));
        return;
      }
      ...
      let targetProductLine = currentProductLine;

      if (result.productLineId) {
        if (
          !targetProductLine ||
          targetProductLine.id !== result.productLineId
        ) {
          const matchingLine = availableProductLines.find(
            (line) => line.id === result.productLineId,
          );
          ...
          if (!matchingLine) {
            set((state) => ({
              ...state,
              skuSearchError:
                "SKU refers to a product line that is not available in this session.",
            }));
            return;
          }

          await loadProductLineOptions(matchingLine);
          targetProductLine = matchingLine;
        }
      }

      const refreshedState = get();
      const { currentConfig } = refreshedState;

      if (!currentConfig) {
        set((state) => ({
          ...state,
          skuSearchError: "Configuration is not initialized; cannot apply SKU.",
        }));
        return;
      }

      const mergedConfig: ProductConfig = { ...currentConfig };

      Object.entries(result.configuration).forEach(([field, value]) => {
        if (value !== undefined && value !== null) {
          (mergedConfig as Record<string, unknown>)[field] = value;
        }
      });
      ...
      if (targetProductLine && postUpdateState.currentConfig) {
        await recomputeFiltering(
          targetProductLine,
          postUpdateState.currentConfig,
        );
      }

      if (result.productId) {
        try {
          const { getProductById } = await import("../../services/supabase");
          const product = await getProductById(result.productId);
          if (product) {
            await setCurrentProduct(product as DecoProduct);
          }
        } catch (error) {
          console.error("Failed to load product for SKU application:", error);
        }
      }
    },
