...
describe("Component Mapping System", () => {
  ...
  describe("mapConfigurationUIToComponent", () => {
    it("should map frame_colors collection to correct component config", () => {
      ...
      const result = mapConfigurationUIToComponent(
        configUI,
        mockProductOptions,
      );
    ...
    it("should throw error for unsupported UI type", () => {
      ...
      expect(() =>
        mapConfigurationUIToComponent(configUI, mockProductOptions),
      ).toThrow(
        "Unsupported UI type: unsupported_type for collection: frame_colors",
      );
    });
    ...
    it("should throw error for unmapped collection in options key", () => {
      ...
      expect(() =>
        ...
        "No options key mapping found for collection: unknown_collection",
      );
    });
  ...
  describe("filterAvailableCollections", () => {
    it("should filter to only collections with available options", () => {
      ...
      const result = filterAvailableCollections(
        testConfigUI,
      ...
      expect(result.map((r) => r.collection)).not.toContain(
        "frame_thicknesses",
      );
  ...
  describe("validateComponentMappings", () => {
    ...
    it("should detect unsupported UI types", () => {
      const invalidUITypeConfigUI = [
        {
          id: "1",
          collection: "frame_colors",
          ui_type: "unknown_type",
          sort: 1,
        },
        {
          id: "2",
          collection: "mirror_styles",
          ui_type: "another_unknown",
          sort: 2,
        },
      ];
      ...
      expect(result.errors).toContain(
        "Unsupported UI type: unknown_type for collection: frame_colors",
      );
      expect(result.errors).toContain(
        "Unsupported UI type: another_unknown for collection: mirror_styles",
      );
    });
    ...
    it("should detect missing collection mappings", () => {
      const unmappedCollectionConfigUI = [
        {
          id: "1",
          collection: "unknown_collection_1",
          ui_type: "button_grid",
          sort: 1,
        },
        {
          id: "2",
          collection: "unknown_collection_2",
          ui_type: "single",
          sort: 2,
        },
      ];
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Missing options key mapping for collection: unknown_collection_1",
      );
      expect(result.errors).toContain(
        "Missing config key mapping for collection: unknown_collection_1",
      );
      expect(result.errors).toContain(
        "Missing title mapping for collection: unknown_collection_1",
      );
      expect(result.errors).toContain(
        "Missing options key mapping for collection: unknown_collection_2",
      );
      expect(result.errors).toContain(
        "Missing config key mapping for collection: unknown_collection_2",
      );
      expect(result.errors).toContain(
        "Missing title mapping for collection: unknown_collection_2",
      );
    });
  });
