...
export async function applyRulesComplete(
  ...
  try {
    ...
    const ruleContext = {
      ...
      mirror_style: currentConfig.mirrorStyle ? parseInt(currentConfig.mirrorStyle) : undefined,
      light_direction: currentConfig.lighting ? parseInt(currentConfig.lighting) : undefined,
      frame_thickness: currentConfig.frameThickness ? parseInt(currentConfig.frameThickness) : undefined,
      mounting: currentConfig.mounting ? parseInt(currentConfig.mounting) : undefined,
      driver: currentConfig.driver ? parseInt(currentConfig.driver) : undefined,
      frame_color: currentConfig.frameColor ? parseInt(currentConfig.frameColor) : undefined,
      color_temperature: currentConfig.colorTemperature ? parseInt(currentConfig.colorTemperature) : undefined,
      light_output: currentConfig.lightOutput ? parseInt(currentConfig.lightOutput) : undefined,
      accessories: Array.isArray(currentConfig.accessories) ? currentConfig.accessories.map((a: any) => parseInt(a)) : undefined
    };
    ...
    for (const rule of rules) {
      ...
      if (evaluateRuleConditions(rule, ruleContext)) {
        if (import.meta.env.DEV) {
          console.log(`✅ Rule "${rule.name}" MATCHED - extracting disabled options`);
    ...
    if (value && typeof value === 'object') {
      ...
      if (collection) {
        if ((value as any)._eq !== undefined) {
          ...
          if (import.meta.env.DEV) {
            console.log(`⚙️ Rule sets ${key} = ${setValue}, will set value and disable alternatives in ${collection}`);
          }
          ...
          if (import.meta.env.DEV) {
            console.log(`⚙️ Rule excludes ${key} = ${excludeValue}, will disable this option in ${collection}`);
          }
        ...
        extractRuleActionsResults(value, disabledOptions, setValues, [...path, key]);
      }
  }
}
